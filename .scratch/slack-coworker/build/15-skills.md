...
- [x] Documentation states that a resource reached by Skill is **outside layer 2** — the deny-list and inventory pin cover the MCP tool path, not the shell — so the credential is the whole boundary and must be genuinely scoped. A read-only database role is a requirement, not a recommendation
- [x] Tests cover: a Job reading a Skill and acting on it; a Job failing to write to the Skills path; and a Skill containing a literal secret being flagged by whatever check the setup story ships, if any
