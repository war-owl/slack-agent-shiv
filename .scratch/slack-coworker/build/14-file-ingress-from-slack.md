...
**What to build:** A person shares a CSV, log dump, JSON export, document, or image in a
Slack Thread, @-mentions the coworker in that message or a later reply, and asks it to
analyse the thing. Supported files from the Thread land inside the Job's sandbox workspace
and the coworker is told they are there. Without this, the only way to get data into a Job
is pasting it as message text, which caps the useful size at a few kilobytes and mangles
anything tabular.
...
- Whether Codex CLI can read **images** when driven through `codex exec`.
...
- [x] Supported files shared earlier in the Thread are retrieved through paginated
  `conversations.replies`, bounded at the triggering message so a queued Job cannot absorb
  later files
- [x] PNG, JPEG, GIF, and WebP files are passed to Codex through the SDK's `local_image`
  channel, not merely mentioned as filesystem paths
...
- [x] An unsupported historical file is skipped so it cannot permanently block every
  later Job; an unsupported file on the triggering mention still fails honestly
...
a live Socket Mode delivery check remains external verification.

**Extended 2026-07-30.** Each Job reads the Thread through `conversations.replies`, follows
its cursor, and includes files no later than the mention that created the Job. Files already
present on the mention are merged by Slack file id, avoiding a duplicate download when the
history response includes that same message. Common Office and OpenDocument formats are
supported alongside the original data/document set. PNG, JPEG, GIF, and WebP inputs use the
Codex SDK's structured `local_image` input, which the installed SDK forwards to
`codex exec --image`.
