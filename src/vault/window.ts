...
import { changeVerb } from "../writes/classify.ts";
import type { VaultChangeLog, VaultChangeRecord } from "./change-log.ts";
 ...
 * provenance onto the Notes that changed, and append a server-log record carrying the
 * diff for each one. That is the whole account of what the coworker did to the human's memory,
 ...
 * after it. Accounting at each point preserves when the work and closing pass changed
 * memory, and a slow closing pass delays nobody because the answer has already gone out.
 *
 ...
 * knows it did, and then the log record says the change may not be this Job's and the
 ...
 * the durable log record and a false `job:` in the file; dropping the
 ...
 *
 * An unattributable change is logged once for every overlapping window that observed it.
 * The hedge is intentional: the shared filesystem establishes that the change happened,
 * but cannot establish which concurrent Job made it.
 */
...
export interface VaultWindow {
   ...
   */
  settle(changeLog: VaultChangeLog): Promise<void>;
...
export async function openVaultWindow(deps: VaultWindowDeps): Promise<VaultWindow> {
  ...
  return {
    async settle(changeLog: VaultChangeLog): Promise<void> {
      ...
      try {
        ...
        // the log shows has to be what the file now holds, frontmatter included, or an
        // operator comparing the record against Obsidian finds two different files. Diffed
        ...
        for (const record of vaultChangeRecords(recorded, attributable, deps)) {
          await changeLog.append(record);
        }
      } catch (error) {
        deps.log.warn(
          `Could not read the Vault at ${deps.notesDir} to see what changed: ` +
            `${reasonFor(error)}. Any Notes written during Job ${deps.jobId} are ` +
            "unrecorded in the Vault change log.",
        );
      }
    },
...
/**
 * Every Vault change, as an append-only server-log record.
 *
 * Anything that changed here changed something a person owns and reads, whether a
 * file-editing tool did it or a shell redirection nothing could see. It deliberately does
 * not become a Slack Write receipt: memory bookkeeping belongs in the server log.
 */
function vaultChangeRecords(
  changes: readonly VaultChange[],
  attributable: boolean,
  deps: Pick<VaultWindowDeps, "thread" | "jobId">,
): VaultChangeRecord[] {
