...
write classification, exact disabled-tool policy, and startup connectivity. It does not own
...
Tool inventories may evolve without blocking startup. Operators add exact names to
`disabledTools` when they want further exclusions and update `writeTools` when new mutating
tools should appear in the Slack audit.
...
- GitHub MCP Writes are classified exactly from engine tool-call events rather than
  inferred from shell strings.
